...
helper-id : {Γ : Ctx C×D} {T : Ty Γ} {c : Ob C} → T ᵗʸ⟨ c ⟩ˡ [ Γ ˢ⟪ hom-id C ⟫ˡ ] ≅ᵗʸ T ᵗʸ⟨ c ⟩ˡ
helper-id {T = T} = ≅ᵗʸ-trans (ty-subst-cong-subst ≅ˢ-id-const-substˡ (T ᵗʸ⟨ _ ⟩ˡ)) (ty-subst-id (T ᵗʸ⟨ _ ⟩ˡ))
{-
    func (from helper-id) {d} {γ : Γ ⟨ c ⟩ˡ ⟨ d ⟩} t
  = func (from (ty-subst-cong-subst ≅ˢ-id-const-substˡ (T ᵗʸ⟨ c ⟩ˡ))) {d} {γ} 
    (func (from (ty-subst-id (T ᵗʸ⟨ c ⟩ˡ))) {d} {γ} t)
  = ty-ctx-subst (T ᵗʸ⟨ c ⟩ˡ) (ctx-id (Γ ⟨ c ⟩ˡ)) t 
  = T ᵗʸ⟨ c ⟩ˡ ⟪ hom-id D , trans (ctx-id Γ) (ctx-id Γ) ⟫ t
-}
{-
  ...
  T ᵗʸ⟨ c₂ ⟩ˡ ↣↣↣↣↣↣↣↣↣↣↣↣↣↣↣↣↣↣↣ T ᵗʸ⟨ c₂ ⟩ˡ
                id-trans (T ᵗʸ⟨ c₂ ⟩ˡ)
-}
mor-to-↣-idˡ : {Γ : Ctx C×D} {T : Ty Γ} {c : Ob C} → from helper-id ⊙ mor-to-↣ˡ T (hom-id C) ≅ⁿ id-trans (T ᵗʸ⟨ c ⟩ˡ)
